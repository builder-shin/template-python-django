...
## Purpose
Django 애플리케이션 모듈 모음. `core` 앱이 공통 인프라(ApiViewSet 4-mixin 합성, JWT 인증, 미들웨어 등)를 제공하고, 나머지는 도메인 리소스 앱.

## Subdirectories

| Directory | Purpose |
|-----------|---------|
| `core/` | 공통 인프라 — ApiViewSet (4 mixin 합성), JWT 인증, 예외 처리, 필터, 미들웨어, 믹스인, 유틸리티 (see `core/AGENTS.md`) |
...
├── views.py         # ViewSet (ApiViewSet 상속, allowed_filters/allowed_includes property로 필터/include 선언)
