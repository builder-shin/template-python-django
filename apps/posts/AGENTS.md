...
| `views.py` | PostsViewSet (CoC pattern) — serializer_class CoC auto-inferred; filterset_class dynamically generated from allowed_filters dict. Permissions: AllowAny for list/retrieve, IsAuthenticated for create, IsOwnerOrReadOnly for update/delete. get_index_scope: authenticated→own posts, unauthenticated→published only. allowed_includes: user, comments. upsert by external_id. create_after_init sets user. |
...
- **get_index_scope**: authenticated users see own posts (all statuses); unauthenticated users see PUBLISHED posts only
