...
class TestPostsAPI:
    def test_index_with_auth(self, mock_authenticated, jsonapi_headers):
        Post.objects.create(title="Test 1", content="Content 1", user_id=str(mock_authenticated.id))
        Post.objects.create(title="Test 2", content="Content 2", user_id=str(mock_authenticated.id))
    ...
    def test_index_only_own_posts(self, mock_authenticated, jsonapi_headers):
        Post.objects.create(title="My Post", content="Mine", user_id=str(mock_authenticated.id))
    ...
    def test_show_existing(self, mock_authenticated, jsonapi_headers):
        post = Post.objects.create(title="Show Me", content="Content", user_id=str(mock_authenticated.id))
        client = APIClient()
        client.force_authenticate(user=mock_authenticated)
    ...
    def test_update_own(self, mock_authenticated, jsonapi_headers):
        post = Post.objects.create(title="Old Title", content="Content", user_id=str(mock_authenticated.id))
        client = APIClient()
        client.force_authenticate(user=mock_authenticated)
        payload = {
            "data": {
    ...
    def test_destroy_own(self, mock_authenticated, jsonapi_headers):
        post = Post.objects.create(title="Delete Me", content="Content", user_id=str(mock_authenticated.id))
        client = APIClient()
        client.force_authenticate(user=mock_authenticated)
    ...
    def test_upsert_updates_existing(self, mock_authenticated, jsonapi_headers):
        Post.objects.create(
            title="Existing", content="Old content", user_id=str(mock_authenticated.id), external_id="ext-002"
        )
        client = APIClient()
        client.force_authenticate(user=mock_authenticated)
        payload = {
