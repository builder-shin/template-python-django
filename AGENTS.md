...
| `docs/` | 아키텍처 및 설계 문서 (see `docs/AGENTS.md`) |
...
- Convention over Configuration (CoC) — serializer_class, filterset_class, included_serializers 자동 추론
...
- **CoC 자동 추론**: serializer_class, filterset_class, included_serializers를 앱 경로 기반으로 자동 추론 (명시적 선언 불필요)
- **UserScopedMixin**: user_id 기반 리소스 소유권 검증 (create_after_init, update_after_init, destroy_after_init)
...
Client → Django (Gunicorn) → DRF JSON:API
           ↓                      ↓
    Session/TokenAuth         ApiViewSet (CoC 자동 추론)
