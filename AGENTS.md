...
| `pyproject.toml` | 프로젝트 메타데이터, Ruff/pytest/coverage 설정 (Python 3.12, line-length 120) |
...
| `scripts/` | 셸 스크립트 (setup.sh) |
| `.github/` | GitHub Actions CI 워크플로우 |
...
Client → Django (Gunicorn) → DRF JSON:API
           ↓                      ↓
    Session/TokenAuth       CrudActionsMixin
    (Django 내장 인증)        (라이프사이클 훅)
           ↓                      ↓
    Redis (캐시/Celery)     HookableSerializer
    PostgreSQL (데이터)
```
